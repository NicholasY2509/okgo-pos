...
export default async function TenantAccountLedgerPage({ params }: { params: Promise<{ tenant: string, id: string }> }) {
  const { tenant, id } = await params

  const account = await LedgerAccountService.getById(id)
  if (!account) return notFound()

  // For tenant view, we only show journal entries associated with this specific branch
  const result = await getJournalEntriesByAccountAction(id, tenant)
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <PageHeader
        title={
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" asChild className="-ml-2 h-8 w-8">
              <Link href={`/${tenant}/accounting/coa`}>
        ...
        description={`Melihat semua transaksi untuk akun ${account.type.toLowerCase()} ini di cabang ${tenant}.`}
      />
        ...
        <div className="divide-y">
          {processedEntries.length === 0 ? (
            <div className="p-8 text-center text-muted-foreground">
              Tidak ada transaksi yang tercatat untuk akun ini di cabang ini.
            </div>
          ) : (
